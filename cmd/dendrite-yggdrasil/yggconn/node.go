...
func Setup(sk ed25519.PrivateKey, peerURI, listenURI string) (*Node, error) {
	n := &Node{
...
func (n *Node) KnownNodes() []spec.ServerName {
	...
	for _, node := range n.core.GetSessions() {
		nodes = append(nodes, spec.ServerName(hex.EncodeToString(node.Key)))
	}
	return nodes
}
